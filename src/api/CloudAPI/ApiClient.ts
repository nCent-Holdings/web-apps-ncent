...
class ApiClient {
  ...
  post(requestURL: string, data?: object, wrapPayload: boolean = true): Promise<any> {
    // TODO: Revert this after back-end is updated
    const requestBody = wrapPayload ? { data } : { ...data };
  ...
  async _request({ method, requestURL, params = {}, body = {} }: RequestOptions): Promise<any> {
    ...
    // TODO: Revert this after back-end is updated? - less sure about this one, we may want to leave this as a fallback?
  ...
  async fetch(url: string, options: object, attempts = 2) {
    for (let i = 0; i < attempts; i++) {
      try {
        ...
        if (error && error.name === 'TypeError' && error.message === 'Network request failed') {
  ...
  _getUrl(url: string, params: object = {}): string {
    ...
    return `${this.prefix}/${url}?${queryString}`;
  }

  _getBodyByMethod(method: RequestMethods, body: object): string | undefined {
    if (method !== RequestMethods.GET) {
      return this._getBodyByContentType(this._getContentTypeByMethod(method), body);
    }
  ...
  _getBodyByContentType(contentType: string, body: object): string {
    return contentType === ApiClient.CONTENT_TYPES.JSON ? JSON.stringify(body) : this._encodeQueryString(body);
  }
  ...
  _getContentTypeByMethod(method: RequestMethods): string {
    return method === RequestMethods.GET ? ApiClient.CONTENT_TYPES.X_WWW_FORM_URLENCODED : ApiClient.CONTENT_TYPES.JSON;
  }
